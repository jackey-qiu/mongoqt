...
def connect_mongodb(self, url_template, user_name, password, use_env = False):
    if use_env:
        user_name = os.getenv('MONGODB_USERNAME')
        password = os.getenv('MONGODB_PASSWORD')
        if (user_name == None) or (password == None):
            error_pop_up('One of the following env variables are not existing: MONGODB_USERNAME; MONGODB_PASSWORD')
            return
    url_complete = url_template.format(self.lineEdit_login_name.text(),urllib.parse.quote(self.lineEdit_password.text().encode('utf-8')))
    try:
